        ...
        <div v-if="showFirstPage" class="col-12">
            <div class="row">
                    ...
                    <div class="row">
                        <div class="col-12 col-md-4">
                            <div class="form-group">
                                ...
                                <input type="text" name="email" id="text" :value="user.name" class="form-control"
                                       disabled>
                            </div>
                        </div>
                        ...
                        <div class="col-12 col-md-4 ">
                            <div class="form-group">
                                ...
                                <input type="text" name="number" :value="user.mobile" class="form-control" disabled>
                            </div>
                        </div>
                        <div class="col-12 col-md-4">
                            <div class="form-group">
                    ...
                    <div class="row">
                        ...
                        <div class="col-12 ">
                            <div class="form-group">
                                ...
                                <textarea v-model="description" style="padding:10px;height:150px;width:100%;border:0;background-color:#f5f5f5;"  class="first_page_required">
                                </textarea>
                            </div>
                        </div>
                ...
                <div class="col-12 bg-white p-3 rounded">
                    <div class="row">
                        <div @click="addAmenities(amenity.id)" v-if="amenities.length" :id="'amenity-'+amenity.id"
                             ...
                             class="col-12 col-md-4 text-center  m-1 alert " style="cursor: pointer">
                ...
                <div class="col-12 mt-3">
                    ...
                    <button @click="showSecondPage" type="button" class="btn btn-success w-25"> {{ locale==='en'?'next':'التالى'}}</button>
                </div>
            </div>
        </div>
        <div v-if="!showFirstPage" class="col-12">
            <form class="row">
                ...
                <div class="col-12 bg-white p-3 rounded">
                    ...
                    <div v-if="advertising.video" class="col-md-6">
                        <video width="320" height="240" controls>
                            <source  :src="'/' + advertising.video" type="video/mp4">
...
<script>
    ...
    export default {
        ...
        props: ['user', 'advertising', 'other_image','locale'],
        data() {
            return {
                lat:parseFloat(this.advertising.location_lat),
                long: parseFloat(this.advertising.location_long) ,
                location_lat: this.advertising.location_lat,
                location_long: this.advertising.location_long,
                cities: [],
                areas: [],
                type: this.advertising.type,
                venue_type: this.advertising.venue_type,
				venuetypes:[],
                showFirstPage: true,
                amenities: [],
                selectedAmenities: [],
                city_id: this.advertising.city_id,
                otherPhotoCounts: 6,
                mainImage: this.advertising.main_image ? this.advertising.main_image : '',
                otherImage: [],
                floor_plan: this.advertising.floor_plan ? this.advertising.floor_plan : '',
                video: '',
                advertising_type: this.advertising.advertising_type,
                title_en: this.advertising.title_en,
                price: this.advertising.price,
                description: this.advertising.description,
                number_of_floor: this.advertising.number_of_floor,
                surface: this.advertising.surface,
                number_of_rooms: this.advertising.number_of_rooms,
                number_of_bathrooms: this.advertising.number_of_bathrooms,
                number_of_master_rooms: this.advertising.number_of_master_rooms,
                number_of_miad_rooms: this.advertising.number_of_miad_rooms,
                number_of_balcony: this.advertising.number_of_balcony,
                number_of_parking: this.advertising.number_of_parking,
                area_id: this.advertising.area_id,
                ...
                furnished: 0,
                security: 0,
                floorImageRender: this.advertising.floor_plan ? this.advertising.floor_plan : '',
                mainImageRender: this.advertising.main_image ? this.advertising.main_image : '',
                otherImageRender: [],
                credit_user: [],
                isLoading:false,
        ...
        created() {
            ...
            this.getCreditUser();
            this.getAreas();
            this.advertisingOtherImage();
            this.getVenueTypes();
        },
        methods: {
            advertisingOtherImage() {

                for (var i = 1; i < 7; i++) {
                    if (this.other_image['other_image' + i]) {
                        this.otherImageRender.push({
                            id: i,
                            image: this.other_image['other_image' + i]
                        })
            ...
            advertisingAmenity() {
                if (this.advertising.amenities.length && this.amenities.length) {

                    this.advertising.amenities.forEach(amenity => {
                        // if ($('#amenity-'+amenity.id)){
                        //     console.log( $('#amenity-'+amenity.id).html('<span>sdhehuiwuh</span>'))
                        //     $('#amenity-'+amenity.id).removeClass('alert-info').addClass('alert-success')
                        //
                        // }
            ...
            submit() {
                if (this.mainImage ) {
                    ...
                    this.isLoading = true
                    axios.post('/'+this.locale+'/advertising/update', data).then(res => {
            ...
            getAmenities() {
                axios.get('/'+this.locale+'/amenities').then(res => {
                    ...
                    if (this.amenities.length) this.advertisingAmenity()
            ...
            addAmenities(id) {
                ...
                if (amenity.length) {
                    ...
                    $('#amenity-' + id).removeClass('alert-success').addClass('alert-info')
                } else {
                    $('#amenity-' + id).removeClass('alert-info').addClass('alert-success')
                    this.selectedAmenities.push({
                        id: id
                    })
                }
        ...
        computed: {
            ...
            commercial_shop() {
                if (this.category === 'commercial' && this.venue_type === 'shop') {
                    return true;
                } else return false
            },
            commercial_office() {
                if (this.category === 'commercial' && this.venue_type === 'office') {
                    return true;
                } else return false
            },
            commercial_land() {
