@extends('layouts.admin', ['crumbs' => [
    ...
    __('Update Package') => route('packages.edit',$package)]
, 'title' => __('Update Packages')])
@section('content')
    <div class="card col-md-12 mx-auto">
        <form method="post" action="{{route('packages.update',$package->id)}}" enctype="multipart/form-data" class="form theme-form">
            ...
            <div class="card-footer">
                ...
                <a @if($package->user_type=="individual") href="{{route('packages.individual.index')}}" @else href="{{route('packages.company.index')}}" @endif  class="btn btn-light">{{__('Cancel')}}</a>
            </div>
        </form>
    </div>
@endsection
